    ...
    while task.wait(0.2) do
        ...
        if bestRoom and bestRoomType > 0 then
            ...
            elseif bestRoomType == 5 then
                ...
                local eggTitle = (type(getgenv().Config.TargetSpecificEgg) == "table" and table.concat(getgenv().Config.TargetSpecificEgg, ", ")) or "Egg"
                if Rayfield then
                    Rayfield:Notify({Title = "🎁 Free Egg Odası Bulundu!", Content = mult .. "x " .. eggTitle, Duration = 5, Image = 4483362458})
                end

                if getgenv().Config.AutoBuyFreeEgg then
                    -- TestHatch.lua ile BİREBİR AYNI: Remote'ları döngü ÖNCESINDE al
                    local RS5 = game:GetService("ReplicatedStorage")
                    local Network5 = RS5:WaitForChild("Network")
                    local hatchRemote5 = Network5:FindFirstChild("CustomEggs_Hatch")
                    local CustomEggsCmds5 = nil
                    pcall(function() CustomEggsCmds5 = require(RS5.Library.Client.CustomEggsCmds) end)
                    local maxHatch5 = 1
                    pcall(function() maxHatch5 = require(RS5.Library.Client.EggCmds).GetMaxHatch() or 1 end)

                    if not hatchRemote5 then
                        warn("[FREE EGG] CustomEggs_Hatch bulunamadı, hatch yapılamıyor!")
                    else
                        print("[FREE EGG] Remote bulundu:", hatchRemote5.Name, "| MaxHatch:", maxHatch5)
                    end

                    local teleported5 = false
                    local missCount5 = 0

                    while getgenv().Config.MetaFarmActive and getgenv().Config.AutoBuyFreeEgg do
                        local customUid5 = nil
                        local model5 = nil
                        local closestDist5 = 150  -- TestHatch ile aynı: 150 stud
                        local root5 = getRootPart()

                        if CustomEggsCmds5 and root5 then
                            for uid, eggObj in pairs(CustomEggsCmds5.All()) do
                                if eggObj._position then
                                    local d = (root5.Position - eggObj._position).Magnitude
                                    if d < closestDist5 then
                                        closestDist5 = d
                                        customUid5 = uid
                                        model5 = eggObj._model
                                    end
                                end
                            end
                        end

                        if not customUid5 then
                            missCount5 = missCount5 + 1
                            if missCount5 >= 5 then
                                print("[FREE EGG] 150 stud içinde yumurta yok, çıkılıyor.")
                                break
                            end
                        else
                            missCount5 = 0
                            -- İlk kez: yumurtanın yanına ışınla
                            if not teleported5 and model5 then
                                local r = getRootPart()
                                if r then
                                    r.Anchored = true
                                    r.AssemblyLinearVelocity = Vector3.new(0,0,0)
                                    r.AssemblyAngularVelocity = Vector3.new(0,0,0)
                                    r.CFrame = model5:GetPivot() * CFrame.new(4, 3, 0)
                                    task.wait(0.3)
                                end
                                teleported5 = true
                            end

                            if hatchRemote5 then
                                print("[FREE EGG HATCH] uid:", customUid5, "amount:", maxHatch5)
                                if hatchRemote5:IsA("RemoteEvent") then
                                    pcall(function() hatchRemote5:FireServer(customUid5, maxHatch5) end)
                                else
                                    task.spawn(function()
                                        local ok, res = pcall(function()
                                            return hatchRemote5:InvokeServer(customUid5, maxHatch5)
                                        end)
                                        print("[FREE EGG HATCH] Sonuç:", ok, tostring(res))
                                        if not ok or res == false then
                                            local ok2, res2 = pcall(function()
                                                return hatchRemote5:InvokeServer(customUid5, 1)
                                            end)
                                            print("[FREE EGG HATCH] Fallback:", ok2, tostring(res2))
                                        end
                                    end)
                                end
                            end
            ...
            elseif bestRoomType == 4 then
                ...
                -- TestHatch.lua ile BİREBİR AYNI: Remote'ları döngü ÖNCESINDE bir kez al
                local RS4 = game:GetService("ReplicatedStorage")
                local Network4 = RS4:WaitForChild("Network")
                local hatchRemote4 = Network4:FindFirstChild("CustomEggs_Hatch")
                local CustomEggsCmds4 = nil
                pcall(function() CustomEggsCmds4 = require(RS4.Library.Client.CustomEggsCmds) end)
                local maxHatch4 = 1
                pcall(function() maxHatch4 = require(RS4.Library.Client.EggCmds).GetMaxHatch() or 1 end)

                if not hatchRemote4 then
                    warn("[HATCH] CustomEggs_Hatch bulunamadı!")
                else
                    print("[HATCH] Remote bulundu:", hatchRemote4.Name, "| MaxHatch:", maxHatch4)
                end
                ...
                while getgenv().Config.MetaFarmActive and (getgenv().Config.FindKeepOutEgg or getgenv().Config.AutoBuyFreeEgg or isHybridEggPhase) do
                    ...
                    if expireTs and type(expireTs) == "number" and timeNow > expireTs then
                        if Rayfield then
                            Rayfield:Notify({Title = "Yumurta Yok Oldu!", Content = "Süresi dolduğu için kayboldu.", Duration = 3})
                        end
                    ...
                    -- TestHatch ile aynı: uid bul
                    local customUid4 = nil
                    local eggModel4 = nil
                    local closestDist4 = 150  -- TestHatch ile aynı: 150 stud
                    local root4 = getRootPart()

                    if CustomEggsCmds4 and root4 then
                        for uid, eggObj in pairs(CustomEggsCmds4.All()) do
                            if eggObj._position then
                                local dist = (root4.Position - eggObj._position).Magnitude
                    ...
                    if not customUid4 then
                        ...
                        if missingEggCounter >= 5 then
                            if Rayfield then
                                Rayfield:Notify({Title = "Egg Despawned!", Content = "Yumurta bulunamadı, yeni oda aranıyor...", Duration = 4})
                            end
                        ...
                        missingEggCounter = 0
                        -- İlk kez: yumurtanın yanına ışınla
                        if not hasTeleportedToEgg and eggModel4 then
                            local r = getRootPart()
                            if r then
                                r.Anchored = true
                                r.AssemblyLinearVelocity = Vector3.new(0,0,0)
                                r.AssemblyAngularVelocity = Vector3.new(0,0,0)
                                r.CFrame = eggModel4:GetPivot() * CFrame.new(4, 3, 0)
                                task.wait(0.3)
                            end
                            hasTeleportedToEgg = true
                        end

                        -- TestHatch ile BİREBİR AYNI hatch mantığı
                        if hatchRemote4 then
                            print("[HATCH] uid:", customUid4, "amount:", maxHatch4)
                            if hatchRemote4:IsA("RemoteEvent") then
                                pcall(function() hatchRemote4:FireServer(customUid4, maxHatch4) end)
                            else
                                task.spawn(function()
                                    local success, res = pcall(function()
                                        return hatchRemote4:InvokeServer(customUid4, maxHatch4)
                                    end)
                                    print("[HATCH] Sonuç:", success, tostring(res))
                                    if not success or res == false then
                                        local s2, r2 = pcall(function()
                                            return hatchRemote4:InvokeServer(customUid4, 1)
                                        end)
                                        print("[HATCH] Fallback:", s2, tostring(r2))
                                    end
                                end)
                            end
                        else
                            warn("[HATCH] Remote yok, hatch yapılamıyor!")
                        end
                    end
                    task.wait(1.5)
                end
