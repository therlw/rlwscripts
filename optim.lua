                        ...
                        task.spawn(function()
                            if customHatchRemote then
                                if customHatchRemote:IsA("RemoteEvent") then
                                    ...
                                    print("[HATCH DEBUG] Çoklu açım sonucu:", success, tostring(res))
                                    ...
                                    if (not success or res == false or type(res) == "string") and maxHatch > 1 then
                                        print("[HATCH DEBUG] Tekli açım (Fallback) deneniyor...")
                                        local s2, r2 = pcall(function()
                                            return customHatchRemote:InvokeServer(customUid, 1)
                                        end)
                                        print("[HATCH DEBUG] Tekli açım sonucu:", s2, tostring(r2))
                                    end
                                end
                            end
                        end)
