            ...
            for _, tData in ipairs(radarTargets) do
                ...
                if targetVec then
                    ...
                    if currentRoot then
                        ...
                        -- Anti-Cheat Kick Algılama:
                        -- Eğer 5 saniye içinde buraya ışınlandıysak ve şu an 1000 stud uzaktaysak BİZİ OYUN GERİ ATTI!
                        if getgenv().RadarLastTeleportTime and (os.clock() - getgenv().RadarLastTeleportTime) < 5 then
                            if getgenv().RadarLastTeleportPos and (getgenv().RadarLastTeleportPos - targetVec).Magnitude < 10 then
                                if dist > 1000 then
                                    if getgenv().RLW_Window then
                                        getgenv().RLW_Window:Notify({Title = "🔒 Locked Room!", Content = "You don't have a key! Anti-Cheat kicked us. Exploring for 60s...", Duration = 3})
                                    end
                                    print("[DEBUG-RADAR] ANTI-CHEAT KICK DETECTED! Blacklisting locked room for 60s.")
