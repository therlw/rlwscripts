        ...
        if bestRoom and bestRoomType > 0 then
            ...
            elseif bestRoomType == 5 then
                ...
                if getgenv().Config.AutoBuyFreeEgg then
                    -- AutoBuyFreeEgg açıksa hatch döngüsü çalıştır
                    local Network5 = game:GetService("ReplicatedStorage"):FindFirstChild("Network")
                    local customHatch5 = Network5 and Network5:FindFirstChild("CustomEggs_Hatch")
                    local CustomEggsCmds5 = nil
                    pcall(function() CustomEggsCmds5 = require(game:GetService("ReplicatedStorage").Library.Client.CustomEggsCmds) end)
                    local maxHatch5 = 1
                    pcall(function() maxHatch5 = require(game:GetService("ReplicatedStorage").Library.Client.EggCmds).GetMaxHatch() or 1 end)
                    ...
                    while getgenv().Config.MetaFarmActive and getgenv().Config.AutoBuyFreeEgg do
                        ...
                        if dist5 > 60 then
                            ...
                            if missCount5 >= 5 then
                                print("[FREE EGG] Yumurta bulunamadı, çıkılıyor.")
                                break
                            end
                        else
                            missCount5 = 0
                        ...
                        if uid5 then
                            ...
                            local hatchR = game:GetService("ReplicatedStorage"):WaitForChild("Network"):FindFirstChild("CustomEggs_Hatch")
                            if hatchR then
                                if hatchR:IsA("RemoteEvent") then
                                    pcall(function() hatchR:FireServer(uid5, maxHatch5) end)
                                    print("[FREE EGG HATCH] FireServer uid:", uid5, "amount:", maxHatch5)
                                else
                                    task.spawn(function()
                                        local ok, res = pcall(function() return hatchR:InvokeServer(uid5, maxHatch5) end)
                                        print("[FREE EGG HATCH] InvokeServer sonucu:", ok, tostring(res))
                                        if not ok or res == false then
                                            local ok2, res2 = pcall(function() return hatchR:InvokeServer(uid5, 1) end)
                                            print("[FREE EGG HATCH] Fallback (1li) sonucu:", ok2, tostring(res2))
                                        end
                                    end)
                                end
                            else
                                warn("[FREE EGG] CustomEggs_Hatch bulunamadı!")
                            end
                        end
                        task.wait(1.5)
                    end

                    -- Döngü bitti, toggle'ları kapat
                    getgenv().Config.FindFreeEggRoom = false
                    if Rayfield and Toggle_FreeEgg then Toggle_FreeEgg:Set(false) end
                else
                    -- AutoBuyFreeEgg kapalıysa sadece bildir ve dur
                    getgenv().Config.FindFreeEggRoom = false
